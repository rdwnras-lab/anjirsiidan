...
function SpecialProductFormPage() {
  ...
  const [form, setForm] = useState({
    name: '', slug: '', category_slug: 'website',
    product_info: '', is_active: true, delivery_type: 'auto',
    download_url: '',
  });
  ...
  const removeImage = (idx) => setPreviewImages(prev => prev.filter((_, i) => i !== idx));
  ...
  const save = async () => {
    ...
    if (variants.some(v => !v.name || !v.price)) return setError('Semua paket harus diisi lengkap.');
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-6 space-y-4">
        ...
        <div>
          ...
          <input className={inputCls} placeholder="Landing Page Premium"
            value={form.name}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className={inputCls} placeholder="landing-page-premium" value={form.slug} onChange={setF('slug')} />
          </div>
          <div>
            <label className={labelCls}>Kategori</label>
            <select className={inputCls} value={form.category_slug} onChange={setF('category_slug')}>
        ...
        <div>
          ...
          <select className={inputCls} value={form.delivery_type} onChange={setF('delivery_type')}>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-6 space-y-3">
        ...
        <p className="text-xs text-gray-400">Ditampilkan di halaman detail produk sebagai deskripsi lengkap</p>
        <textarea className={inputCls} rows={7}
          ...
          value={form.product_info} onChange={setF('product_info')} />
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-6 space-y-5">
        ...
        <div>
          ...
          {previewVideo ? (
            ...
            <button onClick={() => vidRef.current?.click()} disabled={uploadingVid}
              ...
              {uploadingVid ? 'Mengupload video...' : '+ Pilih Video dari Galeri'}
            </button>
          )}
        </div>
        ...
        {/* Foto */}
        <div>
          <label className={labelCls}>Foto Preview ({previewImages.length}/20)</label>
          ...
          {previewImages.length < 20 && (
            <button onClick={() => imgRef.current?.click()} disabled={uploadingImg}
              ...
              {uploadingImg ? 'Mengupload foto...' : '+ Pilih Foto dari Galeri (sisa ' + (20 - previewImages.length) + ')'}
            </button>
          )}
            ...
            <div className="grid grid-cols-4 gap-2">
              {previewImages.map((url, i) => (
                <div key={i} className="relative group aspect-square">
                  <img src={url} alt={'img-' + i} className="w-full h-full object-cover rounded-xl" />
        ...
        <div className="space-y-2">
          ...
          {variants.map((v, i) => (
            <div key={i} className="grid gap-2 items-center" style={{gridTemplateColumns:'1fr 130px 32px'}}>
              <input className={inputCls} placeholder="Paket Standar" value={v.name} onChange={e => setVariant(i, 'name', e.target.value)} />
              <input className={inputCls} type="number" min="0" placeholder="500000" value={v.price} onChange={e => setVariant(i, 'price', e.target.value)} />
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-6 space-y-3">
        ...
        <p className="text-xs text-gray-400">Link ini akan dikirim ke email pembeli setelah pembayaran berhasil (Google Drive, GitHub, dsb)</p>
        <input className={inputCls} type="url"
          placeholder="https://drive.google.com/file/d/xxx/view"
          value={form.download_url} onChange={setF('download_url')} />
      </div>

      {error   && <div className="px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-600 dark:text-red-400 font-semibold">{error}</div>}
      {success && <div className="px-4 py-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-sm text-green-600 dark:text-green-400 font-semibold">{success}</div>}
...
export default function SpecialProductFormPageWrapper() {
  return <Suspense fallback={<div className='flex items-center justify-center py-20'><div className='w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin'/></div>}><SpecialProductFormPage /></Suspense>;
}