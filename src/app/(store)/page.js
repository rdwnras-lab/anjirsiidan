...
async function getHomeData() {
  const [catsRes, prodsRes, bannersRes] = await Promise.all([
    supabaseAdmin.from('categories').select('*').eq('is_active', true).not('slug','in','("website","bot","template")').order('sort_order'),
    ...
    <div className="relative min-h-screen">
      ...
      <div style={{ position:'relative' }}>
        ...
        <div style={{ position:'absolute', bottom:'-52px', right:'48px', zIndex:0, pointerEvents:'none', userSelect:'none', display:'flex', flexDirection:'column', alignItems:'center' }}>
          ...
          <div style={{ width:'14px', height:'18px', borderRadius:'50% 50% 48% 48%', background:'radial-gradient(circle at 40% 35%, #7bb8ff 0%, #1d6fff 60%, #0a3fa0 100%)', boxShadow:'0 0 10px 4px rgba(29,111,255,0.65), 0 0 22px 8px rgba(29,111,255,0.3)', position:'relative' }}>
