...
// ── Komponen halaman sukses (dipakai di dua tempat: via polling + via server)
function SuccessPage({ order, thumbnail, keys }) {
  const [copiedKey, setCopiedKey] = useState(null);
  const isAuto     = order.delivery_type === 'auto';
  const hasKeys    = isAuto && keys && keys.length > 0;
  const qty        = order.quantity || 1;

  const copyKey = (text, idx) => {
            ...
            <>
              ...
              {keys.map((k, i) => (
                <div key={i} style={{display:'flex',alignItems:'center',gap:'8px', marginBottom: i < keys.length - 1 ? '8px' : 0}}>
                  <div style={{
                    flex:1, background:'rgba(0,0,0,0.4)', border:'1px solid rgba(34,197,94,0.3)',
                    borderRadius:'10px', padding:'10px 14px',
                    fontFamily:'monospace', fontSize:'0.85rem', color:'#4ade80', fontWeight:700,
                    wordBreak:'break-all',
                  }}>{k.key_content}</div>
                  <button onClick={() => copyKey(k.key_content, i)} style={{
                    flexShrink:0, width:'36px', height:'36px', borderRadius:'10px', cursor:'pointer',
                    background: copiedKey===i ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.07)',
                    border: copiedKey===i ? '1px solid rgba(34,197,94,0.4)' : '1px solid rgba(255,255,255,0.1)',
...
export default function CheckoutClient({ order, initialCompleted = false, orderKeys = [] }) {
  const mountTime = useRef(null);

  const [status,   setStatus]   = useState(initialCompleted ? 'completed' : (order.status === 'completed' ? 'completed' : 'pending'));
  const [timeLeft, setTimeLeft] = useState('');
  const [qrImage,  setQrImage]  = useState('');
  const [copied,   setCopied]   = useState(null);
  const [expired,  setExpired]  = useState(false);
  // Keys untuk tampil di success page saat dipoll (bukan dari server)
  ...
  // Timer — dari sessionStorage agar tahan refresh
  useEffect(() => {
    if (isManual || status === 'completed') return;
    const KEY = `vech_expiry_${order.id}`;
    const WINDOW = PAYMENT_WINDOW_MS;
    const now = Date.now();
    let expiry;
    try {
      ...
      if (stored) {
        ...
        if (t > now - WINDOW && t <= now + WINDOW + 60000) { expiry = t; }
        else { sessionStorage.removeItem(KEY); }
      }
    } catch {}
    if (!expiry) {
      expiry = now + WINDOW;
      try { sessionStorage.setItem(KEY, String(expiry)); } catch {}
    }
    mountTime.current = expiry;

    const tick = () => {
      const diff = (mountTime.current || expiry) - Date.now();
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === 'completed') {
        // Fetch keys jika produk otomatis
        if (order.delivery_type === 'auto') {
          const keyRes = await fetch(`/api/order-keys?orderId=${order.id}`);
          if (keyRes.ok) {
            const keyData = await keyRes.json();
            setFetchedKeys(keyData || []);
          }
        }
        setStatus('completed');
      }
    } catch {}
  }, [order.id, order.delivery_type, status, expired]);
  ...
  // ── Berhasil
  if (status === 'completed') {
    return <SuccessPage order={order} thumbnail={thumbnail} keys={fetchedKeys} />;
  }

  // ── Expired
  if (expired) return (
    <div style={{maxWidth:'448px',margin:'0 auto',padding:'80px 16px 40px',textAlign:'center'}}>
      <div style={{width:'80px',height:'80px',borderRadius:'50%',background:'rgba(239,68,68,0.12)',border:'1.5px solid rgba(239,68,68,0.3)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px'}}>
      ...
      <p style={{margin:'0 0 28px',fontSize:'0.85rem',color:'rgba(255,255,255,0.5)',lineHeight:1.6}}>
        ...
        telah habis. Silakan buat pesanan baru.
      </p>
      <a href={`/products/${order.product_id}`} style={{display:'inline-block',padding:'13px 32px',borderRadius:'14px',background:'linear-gradient(135deg,#1d6fff,#1450cc)',color:'#fff',fontWeight:800,fontSize:'0.9rem',textDecoration:'none',boxShadow:'0 4px 20px rgba(29,111,255,0.35)'}}>
        Buat Pesanan Baru
      </a>
      <div style={{marginTop:'16px'}}>
        <a href="/" style={{color:'rgba(255,255,255,0.35)',fontSize:'0.8rem',textDecoration:'none'}}>Kembali ke Beranda</a>
      </div>
    </div>
  );
  ...
  return (
    <div style={{maxWidth:'448px',margin:'0 auto',paddingBottom:'40px'}}>
      ...
      <div style={{position:'relative',overflow:'hidden',background:'#0f1e5a',padding:'32px 20px 44px',textAlign:'center'}}>
        <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(29,111,255,0.3) 0%,transparent 50%,rgba(29,111,255,0.2) 100%)',backgroundSize:'300% 300%',animation:'bgP 5s ease infinite'}}/>
        {[{w:90,h:90,t:-20,l:-20,c:'rgba(29,111,255,0.15)',a:'oP0'},{w:60,h:60,b:-15,r:-10,c:'rgba(96,165,250,0.12)',a:'oP1'},{w:40,h:40,t:30,r:'30%',c:'rgba(29,111,255,0.1)',a:'oP2'}].map((o,i)=>(
          <div key={i} style={{position:'absolute',width:o.w,height:o.h,borderRadius:'50%',background:o.c,top:o.t,left:o.l,right:o.r,bottom:o.b,animation:`${o.a} ${4+i}s ease-in-out infinite`,filter:'blur(2px)'}}/>
        ))}
        <div style={{position:'absolute',bottom:-2,left:0,right:0,height:'32px',background:'#0a1628',clipPath:'ellipse(60% 100% at 50% 100%)'}}/>
        <div style={{width:'60px',height:'60px',borderRadius:'50%',background:'rgba(29,111,255,0.2)',border:'1.5px solid rgba(96,165,250,0.4)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px',position:'relative',zIndex:1,animation:'iPulse 2.5s ease-in-out infinite'}}>
        ...
        {!isManual && timeLeft && (
          <div style={{position:'relative',zIndex:1,marginTop:'14px'}}>
            <span style={{display:'inline-flex',alignItems:'center',gap:'7px',background:'rgba(0,0,0,0.4)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'999px',padding:'5px 16px',fontWeight:800,fontSize:'0.9rem',fontFamily:'monospace',color:'#fbbf24'}}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              {timeLeft}
            </span>
          </div>
        )}
        <style>{`
          ...
          @keyframes iPulse{0%,100%{box-shadow:0 0 0 0 rgba(29,111,255,0.5)}50%{box-shadow:0 0 0 14px rgba(29,111,255,0)}}
          @keyframes oP0{0%,100%{transform:translate(0,0)}50%{transform:translate(10px,8px)}}
          @keyframes oP1{0%,100%{transform:translate(0,0)}50%{transform:translate(-8px,-6px)}}
          @keyframes oP2{0%,100%{transform:translate(0,0)}50%{transform:translate(5px,10px)}}
        `}</style>
      </div>
      ...
      <div style={{padding:'20px 16px 0',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:'16px',padding:'16px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}}>
            ...
            <span style={{color:'#fff',fontWeight:700,fontSize:'0.85rem'}}>{payMethodDetail ? payMethodDetail.provider : 'QRIS'}</span>
          </div>
          ...
          <div style={{display:'flex',gap:'10px',marginBottom:'14px'}}>
            <div style={{flex:1}}>
              <p style={{color:'rgba(255,255,255,0.45)',fontSize:'0.8rem',margin:'0 0 6px'}}>Status Pembayaran</p>
              <span style={{display:'inline-block',padding:'4px 12px',borderRadius:'7px',fontSize:'0.72rem',fontWeight:800,letterSpacing:'0.06em',background:'rgba(251,191,36,0.1)',color:'#fbbf24',border:'1px solid rgba(251,191,36,0.25)'}}>UNPAID</span>
            </div>
            <div style={{flex:1}}>
              <p style={{color:'rgba(255,255,255,0.45)',fontSize:'0.8rem',margin:'0 0 6px'}}>Status Transaksi</p>
              <span style={{display:'inline-block',padding:'4px 12px',borderRadius:'7px',fontSize:'0.72rem',fontWeight:800,letterSpacing:'0.06em',background:'rgba(251,191,36,0.1)',color:'#fbbf24',border:'1px solid rgba(251,191,36,0.25)'}}>PENDING</span>
            </div>
          </div>
          ...
          {isManual && payMethodDetail ? (
            <>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'16px'}}>
                <div style={{width:'44px',height:'44px',borderRadius:'10px',flexShrink:0,overflow:'hidden',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',padding:'4px'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
                <div style={{flex:1,background:'rgba(0,0,0,0.35)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'10px',padding:'10px 14px',fontFamily:'monospace',fontSize:'1rem',color:'#e8f4ff',fontWeight:700,letterSpacing:'0.08em'}}>{payMethodDetail.account_number}</div>
                <button onClick={()=>{navigator.clipboard?.writeText(payMethodDetail.account_number);setCopied('acc');setTimeout(()=>setCopied(null),2000);}} style={{flexShrink:0,width:'40px',height:'40px',borderRadius:'10px',cursor:'pointer',background:copied==='acc'?'rgba(16,185,129,0.15)':'rgba(255,255,255,0.07)',border:copied==='acc'?'1px solid rgba(16,185,129,0.4)':'1px solid rgba(255,255,255,0.1)',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.2s'}}>
                  {copied==='acc' ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>}
                </button>
              </div>
              ...
              <p style={{color:'rgba(255,255,255,0.3)',fontSize:'0.75rem',textAlign:'center',margin:'12px 0 0'}}>Konfirmasi ke admin dan kirim bukti transfer.</p>
            </>
          ) : isManual ? (
            <>
