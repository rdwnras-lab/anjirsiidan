...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <aside className={`sidebar-drawer ${sidebarOpen ? 'open' : ''}`}>
        ...
        {session ? (
          ...
          <div className="px-4 py-4">
            <button onClick={() => { setLoginOpen(true); closeSidebar(); }}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm text-white transition-all"
              style={{background:'linear-gradient(135deg,#1d6fff,#1450cc)', border:'none', cursor:'pointer'}}>
      ...
      {loginOpen && (
        <>
          <div style={{position:'fixed',inset:0,zIndex:50,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)'}} onClick={()=>setLoginOpen(false)}/>
          <div style={{
            position:'fixed',inset:'0',zIndex:51,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',
          }}>
            <div style={{
              width:'100%',maxWidth:'340px',borderRadius:'20px',
              background:'linear-gradient(135deg,#0d1e3d,#0a1628)',
              border:'1px solid rgba(255,255,255,0.1)',
              boxShadow:'0 24px 64px rgba(0,0,0,0.5)',
              padding:'28px 24px',
            }}>
              <h2 style={{margin:'0 0 6px',fontWeight:900,fontSize:'1.2rem',color:'#fff',textAlign:'center'}}>Masuk ke Vechnost</h2>
              <p style={{margin:'0 0 24px',fontSize:'0.8rem',color:'rgba(255,255,255,0.45)',textAlign:'center'}}>Pilih metode login kamu</p>
              <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                {/* Discord */}
                <button onClick={()=>{signIn('discord');setLoginOpen(false);}}
                  style={{display:'flex',alignItems:'center',gap:'14px',padding:'14px 18px',borderRadius:'14px',background:'#5865F2',border:'none',cursor:'pointer',width:'100%',transition:'opacity 0.2s'}}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="white"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.001.022.015.04.036.053a19.894 19.894 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg>
                  <div style={{textAlign:'left'}}>
                    <p style={{margin:0,fontWeight:800,color:'#fff',fontSize:'0.9rem'}}>Discord</p>
                    <p style={{margin:'2px 0 0',fontSize:'0.72rem',color:'rgba(255,255,255,0.65)'}}>Login dengan akun Discord</p>
                  </div>
                </button>
                {/* Google */}
                <button onClick={()=>{signIn('google');setLoginOpen(false);}}
                  style={{display:'flex',alignItems:'center',gap:'14px',padding:'14px 18px',borderRadius:'14px',background:'#fff',border:'1px solid #e5e7eb',cursor:'pointer',width:'100%',transition:'opacity 0.2s'}}>
                  ...
                  <div style={{textAlign:'left'}}>
                    <p style={{margin:0,fontWeight:800,color:'#111',fontSize:'0.9rem'}}>Google</p>
                    <p style={{margin:'2px 0 0',fontSize:'0.72rem',color:'#6b7280'}}>Login dengan akun Google</p>
                  </div>
                </button>
              </div>
              <button onClick={()=>setLoginOpen(false)} style={{marginTop:'16px',width:'100%',padding:'10px',borderRadius:'10px',background:'transparent',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.45)',fontSize:'0.82rem',cursor:'pointer',fontWeight:600}}>
                Batal
              </button>
            </div>
          </div>
